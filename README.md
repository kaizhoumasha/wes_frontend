...
在「业务管理 → 作业线管理」（`/biz/worklines`）点击「配置」，在同一工作区切换「基础配置」与「业务配置」。两个分区独立保存。

1. 在「基础配置」先关联本线物理设备，再添加实际工作位，填写工作位编码、名称与执行位置编码（WMS / RCS），按需设置容量、货架属性和关联设备，然后保存。
2. 在「业务配置」先选择插件，再按「设备插槽 → 本线设备」「工作位插槽 → 本线工作位」完成绑定。仅显示本线已保存的资源；缺少匹配资源时点击「前往基础配置」补充。切换插件清空旧插槽绑定，保留基础资源。
3. 草稿允许暂缺绑定，页面显示绑定进度；保存不会启动工作线，也不代表业务已就绪。启动前须满足全部必需插槽和启用检查。切换分区或关闭前会提示处理未保存修改，保存后自动读取最新版本。
...
以下截图来自本机 Mock 环境数据，展示统一工作区，不代表现场验收。
