...
在「业务管理 → 作业线管理」（`/biz/worklines`）点击「配置」，进入独立配置页面（`/biz/worklines/:id/configuration`）。基础与业务配置按「关联设备 → 选择插件 → 配置插槽」完成；设备联调使用单独的「ECS_TEST」分区。
...
4. 已启用工作线的基础与业务配置只读，停用并完成现场清线后才能修改；版本冲突时重新打开，读取最新配置。
5. 在「ECS_TEST」分区选择来源设备后，可读取该设备的默认值作为表单预填。保存或清除设备默认值不会更改 WorkLine 规则；只有显式应用规则才会更新 WorkLine 配置，应用操作不会启动本线，运行时只使用 WorkLine 已应用的规则。WorkLine 运行中可保存或清除设备默认值，但修改 ECS_TEST 规则前须先按既有流程停线。

工作位插槽绑定本线工作位编码，由后端在运行时解析为实际执行位置；设备插槽绑定独立设备编码，设备可以共用 ECS 地址。页面不预设分拣线工作位模板或扫码设备数量。

以下截图来自本机 Mock 环境数据，保留旧版双分区布局供参考；当前页面的基础与业务配置采用上述三步骤，ECS_TEST 分区不在截图中，不代表现场验收。
