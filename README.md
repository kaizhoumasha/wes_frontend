...
- 在「运维工具 → 运输诊断」（`/ops/transport-diagnostics`）查看冻结等待期限、发送次数、发布进度、待应用证据与资源占用；持有 `ops:transport-callback-receipt:read` 时可按回调身份查询收据。收据查询独立于所选任务，接收、发布和物理完成应分别判断。
...
在「业务管理 → 作业线管理」（`/biz/worklines`）分别打开「基础配置」与「业务装配」。基础资源通常只在建线或改造时调整，插件绑定单独保存。

1. 在「基础配置」添加实际工作位，填写本线工作位编码、名称与执行位置编码（WMS / RCS），按需设置货架属性；选择本线物理设备，并关联到工作位。
2. 在「业务装配」先选择插件，再将插件声明的工作位和设备插槽绑定到已保存的本线资源。缺少资源时回到基础配置补充；切换插件会清空旧插槽绑定。
3. 草稿允许暂缺绑定，启动前必须完成全部插槽。已启用工作线只读，停用并完成现场清线后才能修改；版本冲突时重新打开，读取最新配置。

工作位插槽绑定本线工作位编码，由后端在运行时解析为实际执行位置；设备插槽绑定独立设备编码，设备可以共用 ECS 地址。页面不预设分拣线工作位模板或扫码设备数量。

[基础配置桌面截图](./docs/images/workline-basis-desktop.png)与[业务装配窄屏截图](./docs/images/workline-plugin-mobile.png)均来自本机演示数据，使用粗分插件展示通用配置界面，不代表手工拣料插件或现场验收。
