...
describe('Device static master-data page', () => {
  ...
  it('keeps static topology fields and removes runtime/integration fields', () => {
    ...
    const detailKeys = config.detail?.sections.flatMap(section =>
      section.fields.map(field => field.key)
    )
    expect(detailKeys).toContain('device_code')
    expect(detailKeys).not.toContain('device_role')
    expect(detailKeys).not.toContain('role_index')
  })
