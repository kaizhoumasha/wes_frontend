...
describe('runtime agent browser smoke script', () => {
  ...
  it('routes fixture manifests when the frontend appends query strings', () => {
    const source = readFileSync(
      resolve(process.cwd(), 'scripts/runtime-agent-browser-smoke.sh'),
      'utf8'
    )

    expect(source).toContain('/api/v1/workline/plugins/smoke_single_layer/manifest**')
    expect(source).toContain('/api/v1/workline/plugins/smoke_fallback/manifest**')
  })

  it('expects the fallback fixture to report missing runtime boundary fields', () => {
    const source = readFileSync(
      resolve(process.cwd(), 'scripts/runtime-agent-browser-smoke.sh'),
      'utf8'
    )

    expect(source).toContain('运行态边界字段未加载')
  })
})
