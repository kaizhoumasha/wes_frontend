...
import { formatLiteral, schemaToZod } from '../../../scripts/generate-zod-from-openapi'
...
describe('generate-zod-from-openapi helpers', () => {
  ...
  it('serializes array and object defaults as valid Zod arguments', () => {
    expect(formatLiteral([])).toBe('[]')
    expect(formatLiteral({ enabled: true })).toBe('{"enabled":true}')
  })
  ...
  it('rejects values that violate a generated contract constant', () => {
    ...
    expect(
      _DebugTransportStepConfirmationSchema.safeParse({
        ...baseValue,
