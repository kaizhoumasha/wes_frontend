...
- 工作线列表新增资源活动监控入口，可按位置槽位和设备角色查看实际绑定、活动摘要与资源上下文。
- 新增全线活动对象台账、筛选联动和证据详情，并同步 Plane V2 OpenAPI、生成类型与权限快照。

### Fixed

- 活动对象台账改为占满内容区，避免在右侧上下文栏中裁切资源关联、操作提示、主来源和证据列；窄屏上下文继续使用独立抽屉。
...
- 自动联调页面支持编辑货架工作区、投料口、出料口及四个扫码设备，可配置 KT11 等其他区域并预览实际下发值；第四个扫码设备用于出料确认。
- 后续独立轮次沿用已保存的区域配置及实际回架储位；同步正式后端契约。

### Fixed

- 返库步骤预览改为后端实际使用的 WH05 库区。

## [0.23.0.0] - 2026-09-10

### Changed

- 工作线配置改为独立页面，按关联设备、选择插件、配置插槽三步完成；在插槽步骤补充和编辑工作位。
- 设备选择支持服务端分页、名称或编码搜索、归属筛选及跨页选择，便于管理大量设备。
- 优化配置页层级、列表高度与底部操作区；设备选择弹窗打开时阻止离开页面，保护待选设备和未保存配置。

## [0.22.0.0] - 2026-09-10

### Changed

- 工作线管理统一配置入口，区分基础资源与业务配置：先关联设备和物理工作位，再选择插件并映射设备角色、位置插槽。
- 插槽按插件声明展示可选资源与绑定进度，缺少资源可返回基础配置；支持独立保存、版本刷新、未保存草稿保护及键盘切换。
- 活动工作线保持只读，基础与业务权限隔离；保存、停用和重载期间阻止重复写入，重载失败后禁止使用旧版本继续保存。
- 同步后端人工拣料插件声明及基础 START 合同，补充配置导航、并发保存、权限与失败路径回归测试。

## [0.21.0.0] - 2026-09-09

### Added

- 运输诊断展示冻结等待期限、发送次数、发布进度、待应用证据和资源占用，并支持按回调身份精确查询收据。
- WMS 诊断支持脱离近期日志独立查询可靠发送义务与接收事实，分别受读取权限控制。

### Fixed

- 诊断页在重连后刷新当前对象，丢弃切换或关闭前的迟到响应；窄屏显示完整可靠事实，未生成结果不再误报已发布。

### Changed

- 同步后端 0.41.0.1 的查询合同及权限，明确区分接收、应用、发布与物理完成。
...
- 工作线支持独立维护普通工作位、货架工作位、执行位置编码与物理设备关联。
- 业务装配根据所选插件展示工作位和设备插槽，将插槽绑定到本线实际资源。

### Changed

- 基础配置与插件关联分别保存；切换插件清空旧绑定，草稿允许暂缺资源，启动前检查完整性。
- 移除固定分拣线四工作位模板，按插件需求配置不同工作线。
- 同步已合并后端的工作位接口契约与基础配置权限。
...
- 设备接入诊断加载近期回调历史，支持筛选、游标分页和刷新；首次进入及 SSE 重连后回读，按稳定身份合并实时通知。
- 区分回调尝试与旧 Evidence 记录，展示当前应用状态；旧证据不伪造 HTTP 信息，待处理时间允许为空。

### Fixed

- 过滤视图保留最近 200 个 Evidence 的状态，避免先到的应用更新被迟到回调覆盖；断线提示同样受列表容量约束。

### Verification

- 两项新增回归先失败再通过；100 个测试文件、725 项测试通过，lint、生产构建、契约及权限门禁通过。
- 本地浏览器模拟响应验证历史分页、旧证据来源、SSE 状态更新、失败保留列表和 768px 布局；未执行真实设备验收。

## [0.16.0.0] - 2026-09-07

### Added

- 自动联调迁入独立路由与菜单，支持配置 1～1000 轮；每轮仅在后端完成且实际回架储位完整时接续。离开页面或主动停止后不再创建后续轮次。
- 展示已确认回架储位，并回填下一轮配置；保留按权限查询任务、SSE 和持久记录回读。

### Changed

- 从后端 develop 92ffdbaf 冻结 OpenAPI 与权限，重生成类型、元数据和 Zod。

### Verification

- 100 个测试文件、712 项测试通过；lint、生产构建、契约与权限检查通过，重复生成无差异。
- 本地浏览器模拟响应验证独立路由、多轮实际槽位接续与 768px 布局；不代表真实设备验收。

## [0.15.0.0] - 2026-09-07

### Changed

- 从后端 develop 提交 cdbc63f0 正式冻结 OpenAPI 与权限，并同步类型、元数据和 Zod；Transport 统一使用 bin_code。
- 工作线展示当前插件与版本，START 仅提交当前 version；丢失响应或版本冲突后重读，不自动重新启动。
- 设备管理突出物理身份与接入地址；业务角色绑定由工作线配置维护。设备接入诊断展示命令冻结的发送合同。

### Fixed

- Transport 自动联调按当前面 WAIT_SCAN12 的持久记录展示已扫描和待扫描料箱，完成后不再错误显示待扫描。
- 运输接入诊断工具栏在较窄桌面窗口换行，保持自动联调按钮可见。

### Verification

- 98 个测试文件、688 项测试通过；lint、生产构建、契约与权限校验通过，重复生成无差异。
- 独立代码评审与正式生成合同复审通过；本机浏览器 QA 覆盖登录、四页面、空表单校验、Mock 设备枚举与持久自动联调结果。
- 后端完整 Mock 已完成，不代表真实设备或现场业务验收；未部署。
